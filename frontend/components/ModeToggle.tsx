...
export function ModeToggle({ mode, onChange, disabled = false }: ModeToggleProps) {
  return (
    <div className="space-y-3">
      ...
      <div className="relative flex w-full p-1 bg-white/[0.04] rounded-lg border border-white/[0.08] shadow-inner">
        <div
          ...
          style={{
            ...
            boxShadow: '0 0 12px oklch(0.72 0.19 220 / 0.1)',
          }}
        />
        <button
          type="button"
          ...
          className={`relative z-10 flex-1 flex items-center justify-center gap-2 py-2 text-sm font-medium transition-colors duration-200 ${
            mode === 'image' ? 'text-primary' : 'text-muted-foreground hover:text-foreground/80'
        ...
        <button
          ...
          disabled={disabled}
          className={`relative z-10 flex-1 flex items-center justify-center gap-2 py-2 text-sm font-medium transition-colors duration-200 ${
            mode === 'video' ? 'text-primary' : 'text-muted-foreground hover:text-foreground/80'
          } ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
        >
